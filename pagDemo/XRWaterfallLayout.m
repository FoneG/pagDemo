...
- (CGSize)collectionViewContentSize {
    ...
    return CGSizeMake(self.collectionView.bounds.size.width, [self.maxYDic[maxIndex] floatValue] + self.sectionInset.bottom);
}

- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    // fix reloadItemsAtIndexPaths 引起的单个cell的UICollectionViewLayoutAttributes重计算，会造成的布局错乱
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    NSMutableArray *allAttributesInRect = [NSMutableArray arrayWithCapacity:self.attributesArray.count];
    
    for (int index = 0; index < self.attributesArray.count; index++) {
        UICollectionViewLayoutAttributes *attributes = self.attributesArray[index];
